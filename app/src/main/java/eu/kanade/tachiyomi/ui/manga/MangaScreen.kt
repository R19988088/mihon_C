...
class MangaScreen(
    ...
    override fun Content() {
        ...
        MangaScreen(
            ...
            onChapterClicked = { chapter ->
                scope.launch {
                    try {
                        ...
                        viewModel.verifyAndMarkChapterIfComplete(chapter)
                    } catch (e: Exception) {
            ...
            onContinueReading = {
                val unreadChapter = viewModel.getNextUnreadChapter()
                if (unreadChapter != null) {
                    scope.launch {
                        try {
                            // Verify chapter files are complete before opening
                            viewModel.verifyAndMarkChapterIfComplete(unreadChapter)
                        } catch (e: Exception) {
                            logcat(LogPriority.WARN, e) { "Failed to verify chapter completeness" }
                        }
                        context.startActivity(ReaderActivity.newIntent(context, unreadChapter.mangaId, unreadChapter.id))
                    }
                }
            },
