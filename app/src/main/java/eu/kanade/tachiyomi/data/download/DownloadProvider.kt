...
class DownloadProvider(
    ...
    fun findChapterDir(
        ...
        // First try normal lookup with source
        val mangaDir = findMangaDir(mangaTitle, source)
        val chapterDir = getValidChapterDirNames(chapterName, chapterScanlator, chapterUrl).asSequence()
        ...
        // If source is stub/unavailable, search in all source directories
        if (source is StubSource) {
            val mangaDirName = getMangaDirName(mangaTitle)
            val validChapterDirNames = getValidChapterDirNames(chapterName, chapterScanlator, chapterUrl)
            
            downloadsDir?.listFiles()?.forEach { sourceDir ->
                if (sourceDir.isDirectory) {
                    val foundMangaDir = sourceDir.findFile(mangaDirName)
                    if (foundMangaDir != null) {
                        validChapterDirNames.forEach { chapterDirName ->
                            val foundChapter = foundMangaDir.findFile(chapterDirName)
